...
def error(message):
    ...
    print(colored(f"mygit ERR! {message}", Colors.RED))
...
def warn(message):
    ...
    print(colored(f"mygit WARN {message}", Colors.YELLOW))
...
def send_file(args):
    ...
    if not hasattr(args, 'file') or not args.file:
        ...
        print("Usage: mygit send <local_file> <user/repo[:path]>")
        print("Example: mygit send myfile.txt user/myrepo:folder/file.txt")
        sys.exit(1)
...
def copy(args):
    ...
    if not hasattr(args, 'src') or not args.src:
        ...
        print("Usage: mygit copy <source> <destination>")
        print("Examples:")
        print("  Upload: mygit copy ./folder user/repo:remote_folder")
        print("  Download: mygit copy user/repo:file.txt ./local/")
        sys.exit(1)
    ...
    if src.startswith('.') or not ':' in src:
        ...
        if not repo_name:
            ...
            print("Example: mygit copy ./myfile.txt user/myrepo:folder/")
            sys.exit(1)
        ...
        if not repo_name:
            ...
            print("Example: mygit copy user/myrepo:file.txt ./local/")
            sys.exit(1)
...
def remove_file(args):
    ...
    if not hasattr(args, 'file') or not args.file:
        error("File not specified")
        print("Usage: mygit rm <user/repo:path>")
        print("Example: mygit rm user/myrepo:file.txt")
        sys.exit(1)
...
def list_files(args):
    ...
    if repo_arg == ':.':
        ...
        if not repo_name:
            error("Invalid repository format")
            print("Valid formats:")
            print("  mygit ls :.                    (list your repositories)")
            print("  mygit ls user/repo             (list repository root)")
            print("  mygit ls user/repo:folder      (list specific folder)")
            sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  mygit login                                    # Authenticate with GitHub
  mygit ls :.                                    # List your repositories
  mygit ls user/repo                             # List files in repository root
  mygit ls user/repo:folder                      # List files in folder
  mygit send file.txt user/repo                  # Upload file to root
  mygit send file.txt user/repo:folder/          # Upload file to folder
  mygit copy ./folder user/repo:remote_folder    # Upload entire folder
  mygit copy user/repo:file.txt ./local/         # Download file
  mygit rm user/repo:file.txt                    # Remove file
...
Repository format: user/repository[:path]
    ...
    if hasattr(args, 'func'):
        ...
        print("Use 'mygit --help' for more information")
        parser.print_help()
